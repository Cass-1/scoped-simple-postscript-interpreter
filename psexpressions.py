...
class PSString(Expr):
    ...
    def eval(self, ps_env):
        ...
        if isinstance(self.value, str):
            str_val = StringValue(self.value)
            ps_env.opPush(str_val)
        else:
            raise ValueError
...
class PSName(Expr):
    ...
    def eval(self,ps_env):
        ...
        if self.value[0] == '/':
            ps_env.opPush(self.value)
        elif self.value in ps_env.builtin_operators:
            ps_env.builtin_operators[self.value]()
        else:
            val = ps_env.lookup(self.value)
            index = ps_env.staticLink(self.value, len(ps_env.dictstack)-1)
            if isinstance(val, CodeArrayValue):
                val.apply(ps_env, index) 
            # variable
            else:
                ps_env.opPush(val)
